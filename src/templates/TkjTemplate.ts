...
export function renderTkjTemplate(
  invoice: GDTInvoice,
  rawXml?: string,
  ...
  const paymentMethod = invoice.htttoan || 'Đối trừ công nợ';

  return `<!DOCTYPE html>
<html lang="vi">
<head>
  <meta charset="UTF-8">
  <title>HÓA ĐƠN BÁN HÀNG - VÀNG BẠC TKJ - Số: ${escapeHtml(invoice.shdon)}</title>
  <style>
...
<body>
  ${showControls ? getPrintControlsHtml('Hóa đơn bán hàng điện tử - CÔNG TY TNHH TM DV VÀNG BẠC TKJ') : ''}
  ...
  <div class="invoice-outer">
    ...
    <div class="relative-content">
      ...
      <div class="header-grid">
        ...
        <div class="meta-area">
          <div>Ký hiệu (Serial): <strong>${escapeHtml(invoice.khhdon || '2C26MTK')}</strong></div>
      ...
      <table class="tkj-table">
        ...
        <tbody>
          ${items.map((item, idx) => {
            ...
            const unit = item.unit || item.dvt || (idx % 2 === 0 ? 'Món' : 'Cái');
            const name = item.itemName || item.ten || `Sản phẩm vàng trang sức TKJ #${idx + 1}`;
