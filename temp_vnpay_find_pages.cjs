...
async function run() {
  ...
  for (const f of validChunks) {
    ...
    if (c.includes('PagesPortal') || c.includes('Search') || c.includes('tra-cuu') || c.includes('download-xml') || c.includes('download-pdf')) {
      console.log(`Matched in ${f}:`);
      for (const term of ['PagesPortal', 'download-pdf', 'download-xml', '/search', 'tracuu', 'secretCode']) {
        if (c.includes(term)) {
          const idx = c.indexOf(term);
          console.log(`  [${term}]`, c.slice(Math.max(0, idx - 50), Math.min(c.length, idx + 100)));
        }
      }
    }
  }
